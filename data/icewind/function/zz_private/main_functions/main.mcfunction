...
execute as @e[type=minecraft:item_display,tag=scarecrow,limit=8,sort=random] at @s if entity @e[type=minecraft:phantom,distance=..32] as @e[type=minecraft:phantom,distance=..32,limit=6,sort=nearest] at @s run function icewind:zz_private/mob_operations/phantom_motion
